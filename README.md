...
This code example demonstrates how to run face detection using VAAL. The C code in detectfaces.c contains the sample application that runs face detection and demonstrates how it can be modified to fit your needs. This repo contains information on how to run this application on Maivin using Torizon, directly using Docker on Maivin, on an EVK as well as on one's desktop.
...
To create a context with a pre-determined model the following can be written as seen in detectfaces.c lines 138-140
```
...
At this point the detectfaces application will be built and can be run using the provided samples or using your own model and image.
```
./detectfaces -e cpu appconfig_0/face_detection.rtm appconfig_0/test_image.png
